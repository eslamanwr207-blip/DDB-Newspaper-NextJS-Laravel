            ...
            <div>
                ...
                <div>
                    ...
                    <select class="form-select" name="parent" aria-label="Default select example">
                        <option value="{{$category->id}}" selected>{{ $category->parents ? $category->parents->title : 'قسم رئيسي' }}</option>
                    ...
                    @foreach(config('app.languages') as $key=>$lang)
                        ...
                        <div class="tab-pane fade show @if($loop->index == 0) active @endif" id="{{$key}}" role="tabpanel" aria-labelledby="nav-home-tab">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="{{$key}}[title]" id="exampleFormControlInput1" placeholder="name@example.com" value="{{$category->translate($key)->title}}">
                            </div>
                            <div class="mb-3">
                                <label for="exampleFormControlTextarea1" class="form-label">{{$lang}}</label>
                                <textarea class="form-control" name="{{$key}}[description]" id="exampleFormControlTextarea1" rows="3">{{$category->translate($key)->description}}</textarea>
                            </div>
                        </div>
                    @endforeach
                </div>

            ...
            <div>
                ...
                <div>
                    ...
                    <select class="form-select" name="category_id" aria-label="Default select example">
                        <option selected value="{{$post->category_id}}">{{ $post->category ? $post->category->title : 'بدون تصنيف' }}</option>
                    ...
                    @foreach(config('app.languages') as $key=>$lang)
                        ...
                        <div class="tab-pane fade show @if($loop->index == 0) active @endif" id="{{$key}}" role="tabpanel" aria-labelledby="nav-home-tab">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" name="{{$key}}[smalDesc]" id="exampleFormControlTextarea1" rows="3">
                                    {{$post->translate($key)->smalDesc}}
                                </textarea>
                            </div>
                            <div class="mb-3">
                                <label for="exampleFormControlTextarea1" class="form-label">{{$lang}}</label>
                                <textarea class="form-control" name="{{$key}}[description]" id="exampleFormControlTextarea1" rows="3">
                                    {{$post->translate($key)->description}}
                                </textarea>
                            </div>
                        </div>
                    @endforeach
